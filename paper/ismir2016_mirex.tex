...
At a high level, the typical MIREX paradigm operates as follows: %JU: I'd say 'a MIREX run is operated...' I think paradigmn is too much for what is done (in IR we call it the Cranfield paradigm, btw)

\begin{enumerate}
\item Identify some task of interest, such as automatic chord estimation. %JU: shouldn't problem formulation be here?
...
\item Build a corpus of data, with human-annotated ``ground-truth'' annotations for said task. %JU: always human-annotated?
...
\item Evaluate predicted outputs against reference annotations or human judgments. %JU: why annotation != judgment?
\item Repeat steps 5-6 annually. Intermittently repeat step 2 if needed, and steps 3 and 4 if possible. %JU: sometimes 2 as well, to reformulate metrics, for instance
\end{enumerate}
...
Due to a history of litigation from the Recording Industry Artists of America (RIAA), there is a pervasive sense of fear in the MIR community that sharing audio data would almost certainly result in crippling lawsuits~\cite{downie2008music}. %JU: out of ignorance here: only RIAA? Maybe just 'recording industry'
...
More often than not, these algorithms are research prototypes, and are seldom optimized for efficiency or ease-of-use. %JU: and hardly ever work in the first try
...
Similarly, a proposed computational model, $\mathcal{F}$, operates on the same input, producing a prediction, $Z_i$. %JU: maybe \hat{Y}_i instead of Z_i to make it clear that this is an estimate?
One or more comparison functions, $\mathcal{C}^R$, are applied on these two representations, yielding a number of performance measures, $M_i^R$. %JU: a bit lost here...is C a measure and M the score compute with that measure? eg. C=precision M=0.23. If so, I'd rename to measure/metric and score.
This process is repeated over a collection of input-output pairs, $\{X_i, Y_i\} \in \mathcal{D}, |\mathcal{D}|=N$, and the sample-wise measures are aggregated into summary statistics, $S$, the reliability of which generally, although not necessarily, increases with $N$. %JU: not necessarily?
...
Limited insight exists into $\mathcal{A}$, the role that subjectivity may play in a given annotation, or the instructions provided when the annotation was initially performed. %JU cite peeters:ismir2012 here
...
And finally, it is only fair to admit that large scale evaluation is a considerable undertaking with plenty of room for error, but conducting these campaigns in the open makes it easier to detect and diagnose any scientific missteps.%JU I'd say room for error and misinterpretations. Also, I's say 'so conducting' instead of 'but..', and remove 'scientific' because it could also be more programming-related
%JU: all the above problems are identified and discussed in my 2011 and 2013 papers. Should cite the latter imho, even if not now because of the blind thing
...
This is not only detrimental to system development, but also to evaluation, because it is critical to routinely replace the data used for it. %JU had to read this sentence a few times to connect it with the previous one, so went ahead and rewrote it
...
In some cases, this is because the data used for evaluation exists in the wild, as evidenced by one infamous submission in 2011 that achieved near perfect scores in chord estimation having been pretrained on the same data\footnote{http://nema.lis.illinois.edu/nema\_out/mirex2011/results/ace/nmsd2.html}.
% TODO: More detail, converge over multiple iterations.
%JU: do we really want to point fingers?
...
becomes apparent when datasets are expanded, like the introduction of the Billboard dataset to the chord estimation task in 2012~\cite{burgoyne2011expert}.%JU: here's another one. In the similarity dataset there are huge artist biases, but take this: the 700 (10%) of baroque clips are by only 3 authors, and the 700 of classical by only 2!!!
...
%JU: maybe want to finish this section with some sort of "thank you mirex, but we have to move on", so the reader doesn't feel like all we do is criticize?


\section{Open Evaluation of MIR Systems}

Summarizing the previous section, MIREX suffers from three deficiencies that render the situation terminal: %JU: something less agressive than terminal?
one, the financial and human cost is far too great to be sustainable indefinitely;
two, limited transparency undermines the scientific value of the endeavor; %JU: 'lack of transparency limits the scientific..'
...
First is the obvious computational complexity of running $m$ submissions over $n$ inputs. %JU: was capital N above
Second is the less obvious ``human'' complexity of getting the submitted programs to execute properly in a foreign and unknown environment: a task that recently falls on the small number of task captains each year.
...
While the majority of current MIREX tasks do fit into this framework, it precludes tasks requiring symbolic data (score following, symbolic similarity), user interaction (grand challenges, query-by-humming/tapping), or multi-tracks (singing voice separation). %JU: maybe drop here somewhere a 'yet' or 'in this form', meaning that we'll try to incorporate them at some point? Ohterwise it looks like we forget them for good, and a reviewer who is an expert in e.g. symbolic might dislike the whole thing just because of this
...
While there is undoubtedly a strong western bias in these corpora, the same is (almost certainly) the case in MIREX's private data, but using open content at least permits practitioners to quantify the bias. %JU: maybe a little out there, but isn't the whole field biased towards western anyways? I mean...it's not a problem, not because mirex is biased, but because the field is biased. Maybe the folks here at UPF will kill me lol
...
Taken in combination, these indices can be sampled to pre-emptively filter the source collections down to subsets of tracks which are either of interest to a large number of listeners, of interest to a small number of listeners with specific tastes, or representative of particular styles.
