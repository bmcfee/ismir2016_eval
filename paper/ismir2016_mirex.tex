...
% History
The Music Information Retrieval eXcahnge (MIREX) is a framework for the community driven evaluation of MIR algorithms \cite{2008DownieMIREX, 2010DownieMIREX}.
...
As a point of reference, MIREX 2007 required ``nearly 1000 person-hours'' to supervise the execution of 122 algorithms from 40 teams\cite{2008DownieMIREX};
shown in Figure \ref{fig:mirex_participation}, the number of algorithm runs at MIREX has nearly tripled in the years since\footnote{https://www.hathitrust.org/documents/mirex\_htrc\_same\_rda.pdf}.
...
% Hard to do science
% ------------------
Operating costs aside, MIREX has indeed afforded some valuable insight into the research of MIR systems\cite{2008DownieMIREX}.
...
For the sake of visibility into $\mathcal{C}^R$, both the original NEMA infrastructure is open source, and an ongoing community-led effort continues to improve upon these implementations\cite{2014RaffelEval}.
...
These results are misleading due to a fixed, unknown bias in the distribution, which becomes apparent when datasets are expanded, like the introduction of the Billboard dataset to the chord estimation task in 2012\cite{2012BurgoyneExpert}.
...
First is the obvious computational complexity of running $m$ algorithms over $n$ inputs.
Second is the less obvious ``human'' complexity of getting the $m$ algorithms to execute properly in a foreign environment: a task that falls on the small number of ``task captains'' each year.
...
In Kaggle challenges, all input data is made publicly available, and participants submit only the outputs of their systems.
This paradigm effectively resolves the challenges listed above: both computation and manual effort are distributed to the participants, rather than centralized at the evaluation site and task captains, thus reducing the maintenance and administration costs.
...
To mitigate bias, we propose to use a large and diverse corpus of common tracks which are shared across \emph{all tasks}, rather than a collection of small, task-specific datasets as is done in MIREX.
...
The second challenge stems from reduced transparency on behalf of the participants.
While MIREX forces some mild form of transparency by requiring that submissions execute on a remote server beyond the participants' control, the scheme proposed here drops that requirement in favor of data transparency.
However, since MIREX does not require participants to release the source code for their methods, this hardly amounts to a loss of transparency from the perspective of anyone but the task captains, who currently enjoy a somewhat privileged position of access.  
We argue that this trade-off benefits the community at large, since the availability of open data will serve a broader set of interests over time than the description of methods.
...
Luckily, a wealth of data can be obtained from Jamendo~\footnote{\url{http://jamendo.com}} and the Free Music Archive (FMA),\footnote{\url{http://freemusicarchive.org/}} both of which host a variety of music content under either public domain or Creative Commons (CC) licenses.\footnote{\url{https://creativecommons.org/licenses/}}
...
% How are jamendo and FMA simliar, different?
% jamendo: already in dbtune \cite{raimond2008web}
% fma: linked via echo nest
% both have curation
% jamendo is about 5x larger
% fma has public domain as well as cc
% fma features some content by well-known commercial artists
...
While there is undoubtedly a strong western bias in these corpora, this is also (almost certainly) the case in private MIREX data as well, but using open content at least permits practitioners to quantify the bias.

% How do we select a representative, high-quality corpus?
% 1. use curators
% 2. use the charts
% 3. spider out from pre-selected examples ala musiclef or msd
