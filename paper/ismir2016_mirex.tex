...
\begin{abstract}
% MIREX is great!
The Music Information Retrieval Evaluation eXchange (MIREX) has provided a valuable community service by establishing standard datasets, metrics, baselines, methodologies, and infrastructure for comparing MIR methods. 
% ...but we can't keep this up. sad!
While MIREX was able to successfully maintain operations for over a decade, the outlook on its long-term sustainability is bleak. 
% Main problem: the expenditure of effort is intrinsically unsustainable.
The imposed constraint that input data cannot be made freely available to participants necessitates that all algorithms run on centralized computational resources.
This incurs an approximately linear cost with the number of submissions, exacting significant tolls on both human and financial resources, such that the current paradigm is \emph{less} tenable the more people use it.
...
To circumvent these challenges, we propose a distributed, community-centric paradigm for system evaluation, a la Kaggle, built upon the principles of openness, transparency and reproducibility, where users contribute annotations over publicly available audio content in order to participate. 
% Benefits: sustainability, scalability,  
In this document, we outline the goals, benefits, and limitations of such an approach, and 
%describe an alternative framework 
argue that this proposal has the potential to reduce operating costs to sustainable levels, improve scalability, and eventually result in the release of large, open datasets for improving MIR techniques and evaluation methods.
