...
We'ôll then propose an alternative system which should be more sustainable.


\section{MIREX}

MIREX provides a platform for common evaluation of music annotation algorithms.  
The general strategy operates as follows:
\begin{enumerate}
\item Define some task of interest.  Say, chord recognition.
\item Collect human-annotated data for the task.
\item Release a small set of the data for development purposes; retain the rest as private data for evaluation.
\item Invite authors to submit code, which is executed on a private server.  Report the results.
\item Repeat step 4.  Intermittently repeat step 2.
\end{enumerate}
(Note: this ignores some tasks, such as audio music similarity, which require human annotators to rate the outputs of each algorithm.  We'Äôll come back to that later.)

MIREX retains private data sets to stave off the effects of over-fitting to the evaluation set. 
The theory goes, if participants don't see the data, it'Äôs harder to overfit.

It has been argued that retaining private data is more cost-effective, since Step 2. above is typically the most expensive.  
If data sets are not released, they can be reused year upon year.  
(There is also the issue of dealing with obtaining distribution licenses for musical content, which we'll return to below.)  
However, systems will still eventually over-fit by trial and error, so there is no avoiding the data collection issue in the long run.  
Moreover, step 4 requires a considerable amount of volunteer effort to maintain, through the so-called ``útask captains''Äù, as well as various administration and maintenance issues.  
This results in a system that is ultimately unsustainable, as the costs are proportional to participation (which we should hope would grow), rather than data (which would be constant with respect to the number of participants).
...
\begin{itemize}
\item Use large test collections, so that it'Äôs impossible (or impractical) to manually fit to the test
...
\begin{itemize}
\item annotation acquisition
\item content selection and sharing
\item participation / honesty
\end{itemize}

\subsection{Annotated Data Acquisition}
...
To participate in an evaluation, each participant must ``Äúpay''Äù some nominal amount of data credits.  
In the first year, all participants start with a balance of 0, so by participating, they are now in ``Äúdata debt''Äù.  
...
Data credits may also be earned at a discounted rate by auditing or validating previously provided annotations?  We'Äôll need a system in place to authenticate annotators and force independence.

%Potentially, we may institute a cap-and-trade system to exchange data credits for money which can be used to pay for expert annotations.  Not sure about the legal ramifications of implementing this, or how to organize efforts.  Aside from cash exchange, there might be a benefit to having a task exchange: eg, ‚Äúi‚Äôll trade three beat annotations for one chord annotation‚Äù.

There are other concerns here:

\begin{itemize}
\item Standardization and the enforcement of it / them
...
Several recent studies have investigated the use of algorithm disagreement for detecting ``Äúhard''Äù examples, be it for beat tracking \cite{}, structural analysis \cite{}, or chord recognition \cite{}.
...
If we aÄôre careful about selecting these data, we can then compare the outputs across the participating systems to see which ones are ``Äúinteresting''Äù: if the systems generally agree, the example is likely to be easy; at the very least, annotating this track will not help distinguish between these systems.  
...
Each track marked as ``interesting''Äù should be annotated by at least two humans to determine whether there is actual disagreement.  
It can then be annotated in year 2, and enter the evaluation in year 3.  
By having multiple reference annotations for the ``Äúinteresting''Äù tracks, we can compare algorithm agreement to annotator agreement, and distinguish ``difficult''Äù tracks from ``Äúambiguous''Äù tracks.
...
Where does this fit into the proposed ``Open Evaluation Framework'' narrative? Is it a core tenet that comes first, or part of the framework? Or truly an afterthought?
...
If we start getting into the business of slicing down to subsets per task, it will become much easier to determine which tracks are used for which task, and we don'Äôt want that.  
...
If it isn'Äôt open, it isn'Äôt science. 
(Just make it cost money then. Which would require the org to NPO-ize).

\subsection{Coverage, or ``ÄúWhat tasks are left out?''}
...
To get this off the ground, we'ôll need a few things:
...
\section{Misc Refs}
Separate ‚Äúlabels‚Äù for systems depending on what they do (eg. close/open, full/subset of dataset): http://trec.nist.gov/pubs/trec22/papers/WEB.OVERVIEW.pdf 
