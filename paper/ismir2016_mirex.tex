...
However, we note two potential drawbacks to this approach.
First, the test (input) data are openly available to participants, so there is some increased risk of cheating (or honest over-fitting) if participants tune their algorithms to the test set.
To mitigate this problem, we propose to use a large and diverse set of common tracks which are shared across \emph{all tasks}, but only compare participants on hidden, task-dependent subsets.
For a submission to be included in the evaluation, it must cover all common tracks, which reduces the feasibility of participants hand-tuning performance to a specific test set.
