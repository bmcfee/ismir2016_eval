...
To illustrate this reality, consider the standard approach to benchmarking MIR systems, diagrammed in Figure \ref{fig:system_eval}.
An input, $X$, is observed by an annotator, $\mathcal{A}$, producing a ``gold-standard'' output, $Y$.
Similarly, a proposed computational model, $\mathcal{F}$, operates on the same input, producing a prediction, $Z$.
One or more comparison functions, $\mathcal{C}$, are applied on these two representations, yielding a number of performance measures, $M$.
This process is repeated over a collection of input-output pairs, $\mathcal{D}, |\mathcal{D}|=N$, and the sample-wise measures are aggregated into summary statistics, $S$, the reliability of which generally increases with $N$.
...
They do not have access to $X$, making it impossible to diagnose why an algorithm produced $Z$, or determine what should be done to make the system better.
Furthermore, $Y$ is meaningless without $X$, and must be trivially taken on face value.
They also have limited insight into $\mathcal{A}$, the role that subjectivity may play in a given annotation, or the instructions provided when the annotation was initially performed.
