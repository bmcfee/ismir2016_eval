...
The annual tradition of MIREX was established early in the lifetime of ISMIR, drawing inspiration from TREC in natural language processing (NLP).
...
The practicalities of MIREX are selflessly shouldered by the IMIRSEL group at UIUC, and the organization has successfully earned multiple grants to jumpstart the evaluation effort at ISMIR.
...
Note that, as a matter of correctness, some tasks, such as audio music similarity, have evolved to require that human annotators provide subjective judgments over the outputs of each algorithm every year.
More importantly, however, MIREX differs from TREC-style evaluation in one critical way, given in (4) above:
...
In contrast to other machine perceptions domains, such as natural language processing, speech recognition, or computer vision, intellectual property and copyright law imposes stiff penalties on the illegal distribution of recorded music \cite{2008DownieMIREX}.


% Deficiencies
% Operating costs are significant
In the last decade of MIREX though, experience has more than demonstrated that the decision to bring algorithms to data comes at a significant cost.
On the one hand, this is purely a practical challenge, alleviated by enough funding.
Sufficient computational resources are necessary to run the algorithms submitted.
In the spirit of inclusiveness, MIREX places no upper bound on complexity but a 72 hour runtime.
% The cost of hardware and electricity is trivial compared to the human effort involved.
Supervising all this computation is no simple feat, which in and of itself requires significant human effort.
As a point of reference, MIREX 2007, only three years into the project, required ``nearly 1000 person-hours'' to run 122 algorithms from 40 teams to completion \cite{2008DownieMIREX}.
% MIREX2015 saw 145 algorithms from 61 teams.
% As MIREX has diminished in interest, the last decade has consumed 10,000 hours of effort and millions of dollars.
This has resulted in a paradigm that is ultimately unsustainable, as the operating costs increase linearly the more successful the endeavor becomes.

% Hard to do science
At the same time, the practice of witholding input data from participants comes at the expense of many forms of scientific progress.
Granted, as detailed in a review of MIREX \cite{2008DownieMIREX}, some insights into evaluation, methodology, or problem formulation have been gleaned in the lifetime of MIREX.
That said, these advances are bottlenecked through the organization, whereas the participants are largely blind to the inner-workings of the framework and the data used therein.
As outlined by Urbano et al, there are a number of valuable research dimensions, being problem definition, algorithm development, evaluation machinery, interpretation, and learning \cite{2013UrbanoEval}.
Each of these are impeded or, at worst, obfuscated by the algorithm-to-data model, because conclusions cannot be reached in the absence of perspective of the input.

% Data Replenishing
Ironically, due in part to the high operating costs of the algorithm-to-data model, MIREX also suffers from a lack of data.
