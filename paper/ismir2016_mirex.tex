...
\item Repeat step 5 annually. Intermittently repeat step 3, as needed.
\end{enumerate}

% Algorithm-to-data goes astray.
Note that, as a matter of correctness, some tasks, such as audio music similarity or user interfaces, rely on post-hoc human assessment, either by design or necessity.
More importantly, however, MIREX differs from TREC-style evaluation by operating in an ``algorithm-to-data'' model, where facilitators oversee the application of code submissions to privately held data, rather than participants submitting predictions over a freely available dataset.
...
However, experience with MIREX over the last decade has more than demonstrated that the decision to bring algorithms to data comes with fundamental limitations.
...
% This slide deck claims MIREX hit the 300s for years: https://www.hathitrust.org/documents/mirex_htrc_same_rda.pdf
% Not sure what accounting went into this, but it seems to be consistent with the 122 algos from 2007, and probably the more fair comparison.
...
This process is repeated over a collection of input-output pairs, $\{X_i, Y_i\} \in \mathcal{D}, |\mathcal{D}|=N$, and the sample-wise measures are aggregated into summary statistics, $S^R$, the reliability of which generally, although not necessarily, increases with $N$.
...
By the same token, while $N$ varies from task to task, there is no way to guage the distribution of the data or how representative it may or may not be.
...
Traditionally $\mathcal{C}^R$ are implemented with proprietary technologies, i.e. MATLAB, by a handful of individuals with minimal oversight from the community, raising concerns of technical correctness.
...
While this is certainly detrimental for system development, it is absolutely critical to routinely replace the data used for evaluation.
Unsurprisingly, in lieu of a fresh source of annotated data, the community is beginning to over-fit MIREX datasets.
In some cases, this is because the data used for evaluation exists in the wild, as evidenced by one infamous submission that achieved near perfect scores in chord estiamtion via fingerprinting\footnote{sneaky McVicar...}.
In others, the ``hold-out'' data used for evaluation is simply being over-fit by trial and error.
It is widely expected that updating evaluation datasets will cause algorithm scores to drop significantly.
...
This approach suffers from the same lack of transparency as MIREX itself, often quietly introducing issues of methodology and problem formulation, and can hardly be relied upon as a regular resource.
...
two, a lack of transparency undermines the scientific value of the endeavor;
