...
To get this up and running within the year, there are several concrete steps:
\begin{itemize}
\item Establish a robust instrument taxonomy as a community (this was key for ImageNet).
\item Requisition a large plurality of audio content, \eg mirror FMA.
\item Build a web-based annotation tool, linking content to URLs on FMA, that uploads annotations from the browser to a content management system.
\item Consolidate existing datasets, \eg IRMAS, RWC, MedleyDB, into a common format as a development dataset.
\item Implement or collect a few simple algorithms to prioritize content for annotation.
\item Perform data collection, through some combination of paid annotation, crowd-sourcing, and community support.
\item Evaluation server and leaderboard, leveraging mir\_eval and Codalab.
\end{itemize}
