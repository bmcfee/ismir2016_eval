...
The challenges we describe in this document, which our proposed strategy aims to address, are not news to the community.
...
if it were easy, there would be less motivation to develop computational systems.
...
In a lot of ways, the approach taken by MIREX made perfect sense in the early 2000's.
...
First and foremost, creative commons music is now ample, making the dream of sharing data a reality.
% Operating costs are minimal.
Cloud computing is cheap and ubiquitous, any infrastructe costs on the side of the eval organizers / organization are cheap, rent compute cycles (AWS) and storage (Google Drive / Box / DBox).
Additionally, web infrastructure has only improved, such that bringing web-scale data to computation is feasible.
% Tools and technologies are better, JavaScript support, HTML5, python, etc.
We can now easily build annotation tools into the browser, and collect annotations from humans in the wild.
% We're ready for it.
The community has grown, and so have our skills, the number of labs involved, our industrial contacts, and our status.
We are involved in more open source development, and transparency and reproducibility are becoming cornerstones of modern data science.
...
As an initial undertaking of the framework described here, we conclude our proposal by advocating a large-scale instrument identification for ISMIR2017.
% What is the task?
In this task, one or more active instrument classes are predicted from a pre-defined taxonomy over an input audio recording.
Some of the effort in building an instrument taxonomy can be leveraged from WordNet\cite{}, and refined by the community, starting at this year's ISMIR conference.
% Motivations
There are a number of strong motivations for pursuing this particular challenge.
% Good MIR problem, let's finally solve something.
Instrument identification is an important, classic problem in MIR, but is not currently supported by the MIREX framework explicitly.
...
Compared with other music perception tasks, annotation requires a simple, straight-forward interface, \eg ``check all that apply'', and \emph{does} maintain at least a modicum of objectivity, \ie someone either ``is'' or ``is not'' generating sound with a particular instrument\footnote{Granted, synthesizers wreak havoc on any taxonomy; computer vision faces similar issues with natural images of paintings, for example.}.
% Content validity.
Instrument occurrence is independent of popularity, and thus content validity is a non-issue for open source music.
% Epic win for the community.
And finally, computer vision found fantastic success with a parallel endeavor, known as ImageNet \cite{}, in which algorithms detect objects, \ie sources, from natural images.
ImageNet has proven to be an unequivocal win for the field, not only advancing research methods to that specific task, but also sparking other investigations, such as Google's now-famous Deep Dream\cite{}.
