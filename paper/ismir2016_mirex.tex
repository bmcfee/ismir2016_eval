...
While task-dependent runtime limits are placed on algorithm runtime, varying between 6 and 72 hours, MIREX is responsible for months of compute time annually;
% there's a better way to say this, but gotta scram.
note that these ceilings cannot be decoupled from dataset size, and are indicative of the small datasets traditionally used.
The financial burden of computation is a rounding error, however, when compared with the human effort involved.
...
participation in 2015 was comparable, with 145 algorithms from 61 teams.
Extrapolating, the last decade has easily consumed roughly \emph{10,000} person-hours and millions of dollars just bringing algorithms to data.
Not only is this rate unsustainable, but the operating costs only \emph{increase} the more successful the endeavor becomes.
Said differently, the worst thing that could happen to MIREX is growth.
