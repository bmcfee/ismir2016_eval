...
By directing the majority of resources toward the production of open data, the proposed framework will be of value to the greater MIR community in perpetuity, and benefits will not be limited to participants.
...
shown in \cref{fig:mirex_participation}, the number of algorithm runs at MIREX has nearly tripled in the years since.\footnote{\url{https://www.hathitrust.org/documents/mirex\_htrc\_same\_rda.pdf}}
...
\begin{figure}[!t]
 ...
 \caption{Diagram of the standard approach to the benchmarking of MIR systems.}
 %JU: throughout, we should be consistent in calling them comp. models, systems or algorithms.
 \label{fig:system_eval}
\end{figure}
...
In some cases, this is because the data used for evaluation exists in the wild, as evidenced by one submission in 2011 that achieved near perfect scores in chord estimation having been pretrained on the same data\footnote{\url{http://nema.lis.illinois.edu/nema\_out/mirex2011/results/ace/nmsd2.html}}.
...
These results can mislead due to fixed but unknown biases in the distribution, which become apparent when datasets are expanded --- like the introduction of the Billboard dataset to the chord estimation task in 2012~\cite{burgoyne2011expert} --- or as further insight is gained, as in the disclosure of the metadata in the music similarity corpus\footnote{}.
...
\item Incremental evaluation encourages that results are scientifically meaningful by keeping test data fresh, while steadily increasing the size of datasets available for development and evaluation.
\end{itemize}
...
Using large test collections with opaque evaluation sets will limit the practicality of manual participant intervention
...
While the majority of current MIREX tasks do fit into this framework, at present it precludes tasks for which ample input data exists (score following, singing voice separation) or user interaction (grand challenges, query-by-humming/tapping).
...
In the end, systems are ranked as usual, but with some degree of uncertainty which is inversely proportional to the number of available annotations.

In the first round, uncertainty in the performance estimates is maximal due to a small number of initial annotations.
Participants then submit their system's outputs, for which we estimate performance.
New annotations may arrive at any point during (or after) the evaluation round, which will reduce uncertainty.
...
Whether they are both right or wrong does not provide us with any information as to which one has higher accuracy, so there is little to gain from annotating that particular example.
With many systems and complex performance metrics, prioritizing data for annotation is by no means obvious.
...
In the proposed framework, annotations may come from different sources, so it is imperative that we can trust whatever information is submitted as ground truth.
Another line of work is thus the development and enforcement of standards, guidelines and tools to perform annotations.
This will probably involve the development of both desktop and web technologies for creating and managing annotations, as well as appropriate version and quality control mechanisms.
Quality control is particularly important if annotations are collected via crowd-sourcing platforms like Amazon Mechanical Turk.
...
\begin{enumerate}
\item Identify and collect (open) audio data.
...
%  * Any fundraising efforts can be allocated to data collection, rather than operating costs. We've had success raising money, but we could be using those funds differently.
% ---
...
In this section, we elaborate on a number of issues relating to the implementation and overall goals of the proposed framework.

\subsection{Timeliness}

Folks complain about data and evaluation annually.
Why now?
Why is this any different?

We're in a different day and age.
MIREX made a good deal of sense in the early 2000's.
But we do not live in 2003, and the times have changed.
Amazon web services is a thing that's gotten quite cost effective.
Practically no one in industry purchases and maintains their own compute resources.
Bandwidth is cheap and better, so transferring large datasets isn't the hurdle it once was.
The open source audio content game has changed dramatically.
There wasn't even data, never mind \emph{good} data.
As discussed, there exists a large number of tracks that we \emph{can} share.
Web technologies are light-years better, and browser-based annotation is not only feasible but inevitable.
Mechanical Turk is a thing and, though rife with its own challenges, has worked great for a number of problems.
...
Allowing participants to submit predictions, rather than software, may raise the issue of
participant integrity: how can we verify that the predictions are generated by the
process described by the accompanying abstract?
...
The proposed framework requires a continuous source of fresh annotations for each evaluation campaign.
Here, we identify three potential sources for data collection.

First is the traditional route of acquiring funding and paying expert annotators.
This is the most expensive option, but also most likely to produce high-quality data, and may in fact be the only viable path for certain tasks.

Second, for tasks which require minimal training, we can leverage either crowd-sourcing platforms, or seek out enthusiastic user communities that would be willing to participate in \emph{citizen science} for music on a volunteer basis.
Community sites such as Ultimate~Guitar\footnote{\url{http://www.ultimate-guitar.com/}} and Genius\footnote{\url{http://genius.com/}} demonstrate the existence of these communities, though harnessing their enthusiasm may take some effort.

Finally, if no funding can be found to annotate data, we may solicit annotations
directly from participants and the ISMIR community at large.
...
As mentioned in \cref{sec:implementation}, web-based annotation tools will enable volunteer contribution, in addition to any paid annotation labor.

% We've had resources, we just invested it elsewhere.
%One, we've invested a ton of time and money into computation over the last decade.
%MIREX got a Mellon grant for \$1.2M.
%MedleyDB, a well-curated dataset of over 100 multitracks for a number of MIR tasks, cost approximately \$12 a track \cite{bittner2014medleydb}.
%Where would we be if that went toward annotation?
%We've been able to procure funding in the past for projects, and can divert those resources to annotation.
...
\subsection{An Honest Path to Adoption}
