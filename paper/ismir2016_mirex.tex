...
These results are sometimes misleading due to a fixed, unknown bias in the distribution, which becomes apparent when datasets are expanded, like the introduction of the Billboard dataset to the chord estimation task in 2012~\cite{burgoyne2011expert}, or further insight is gained, as in the disclosure of the metadata in the music similarity corpus\footnote{}.
...
\begin{enumerate}
\item Identify open data to include in a corpus for distribution.
\item Define or update a number of human perception tasks, \eg chord transcription.
\item Invest resources in human annotation; optionally, prioritize items by information gain.
\item Release a collection of training data, with annotations, for development purposes, and an over-complete collection of unlabeled data, for evaluation.
\item Invite participants to submit \emph{predictions} over the unlabeled data.
\item Submissions are scored on a private server against hidden annotations or human judgments; summary results are provided at the conclusion of the submission window.
\item Retire (witheld) annotated data, formerly used for evaluation, into the training set.
\item Go to step 2 and repeat. Revisit step 1 if needed.
\end{enumerate}
...
\begin{itemize}
    \item Timeliness
    \item Implementation
    \item On the Integrity of Data and Participants
    \item Meta-Evaluation
    \item Getting Serious about Annotation
    \item An Honest Path to Adoption
\end{itemize}}
...
We ultimately rely on participants to be honest in their methods and reporting.
What's to prevent a participant from manually annotating all potential test tracks?
...
\subsection{Getting Serious about Annotation}
...
MedleyDB, a well-curated dataset of over 100 multitracks for a number of MIR tasks, cost approximately \$12 a track \cite{bittner2014medley}.
