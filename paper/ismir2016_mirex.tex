...
% --------------
\threeauthors%
  {Brian McFee} {Center for Data Science / MARL\\ New York University \\ {\tt brian.mcfee@nyu.edu}}
  {Eric J. Humphrey} {Spotify, Inc. \\ ~\\ {\tt ejhumphrey@spotify.com}}
  {Juli\'{a}n Urbano} {Music Technology Group \\ Universitat Pompeu Fabra \\ {\tt julian.urbano@upf.edu }}
...
\item Construct (and annotate) corpus of data.
...
First, there is no direct access to the reference annotations $Y_i$, and, in most cases, no access to the input $X_i$ as well.
...
Still, without access to the data, it is exceedingly difficult to perform \emph{meta-evaluation}, or compare new metrics on old data, without seeking privileged access to the historical records.
...
Free audio content is significantly easier to acquire now than when MIREX began, and in particular, a wealth of data can be obtained from services like Jamendo~\footnote{\url{http://jamendo.com}} and the Free Music Archive (FMA).\footnote{\url{http://freemusicarchive.org/}}
...
However, using open content at least permits practitioners to quantify this bias.
...
Rather than fully annotate the corpus up front for each task, we plan to adopt an \emph{incremental evaluation} strategy~\cite{carterette2005incremental}.
...
Instead, performance measures are estimated over a subset of annotated tracks, which may itself grow over time.
...
Systems are ranked as usual, but a degree of uncertainty inversely proportional to the number of available annotations.

Initially, uncertainty in the performance estimates will be maximal, due to a small number of available annotations.
Subsequently, as reference annotations are collected and integrated to the evaluation system, they will be compared to the submissions' estimated annotations, and provide incrementally better performance estimates.
...
\item Collect reference annotations, prioritized by disagreement among predictions.
...
In this section, we elaborate on several points relating to the goals and implementation of the proposed framework.
...
%Relatedly, data collection is inherently difficult;
%if it was easy, there would be no need for automatic annotation systems.
This begs an obvious question: why should things change \emph{now}?
...
This approach has been effectively used to collect judgements for the Audio Music Similarity task.

In each case, we will institute a ratification system so that annotations are validated by an independent annotator before being accepted into the evaluation set.
As mentioned in \cref{sec:implementation}, web-based annotation tools will enable volunteer contribution, which we will welcome in addition to paid annotations.
...
\begin{enumerate*}[label=(\roman*)]
\item establish, as a community, a robust instrument taxonomy;
...
\item[and finally,] deploy an evaluation server and leader-board to accept and score submissions.
\end{enumerate*}
