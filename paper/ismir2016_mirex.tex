...
In addition to empirical results reported in individual research articles, community evaluation campaigns --- such as MIREX~\cite{downie2008music}, MusiClef~\cite{orio2011musiclef}, or the MSD Challenge~\cite{mcfee2012million} --- provide a mechanism to benchmark several methods on a common dataset with standardized metrics.
MIREX in particular has earned a special role within the MIR community as the standard point of reference for evaluation.
However, each year, MIREX incurs substantial operating costs, which are ultimately unsustainable.
...
If we, as a community, are to continue hosting regular evaluation campaigns, we will eventually need to find a more efficient and sustainable model.

In this article, we propose an alternative, open framework for community-driven MIR evaluation campaigns.
Our proposal is motivated by three complementary factors.
First, we strive to reduce the cost of running and maintaining the evaluation system.
...
We stress that this document constitutes a \emph{proposal} and not a fully implemented system.  
Our goal in writing this document is to engage with the MIR community at an early stage, and solicit feedback before finalizing the implementation details.
