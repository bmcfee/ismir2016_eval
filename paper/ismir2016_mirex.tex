...
More often than not, these algorithms are research prototypes, rarely work in the first try, and are seldom optimized for efficiency or ease-of-use.
...
Usually, there is no access to any $Y_i$, and in most cases there is no access to $X_i$ either, making it impossible to diagnose why an algorithm produced $\widehat{Y}_i$, or determine what should be done to make $\mathcal{F}$ better.
...
One obvious problem of our proposal is that we start off with a brand new corpus of audio for which, in principle, there are no task-specific annotations yet. Initial community-driven efforts could be set to annotate at least part of it for a first evaluation run, and then keep annotating whatever is left. But this is precisely the scenario we intend to avoid, so we plan to resort to incremental evaluation. 
In our setting we will have an incomplete ground truth, that is, some input examples will not have associated annotations. The idea is to develop methods to estimate these missing annotations from all available sources of information, such as existing annotations, metadata, system outputs, etc. Annotations will therefore be defined probabilistically, over the space of possible labels, so that missing annotations will have some degree of variance (uncertainty); existing annotations will have zero-variance. With an appropriate reformulation of our performance measures, we can incorporate these estimates of the annotations and produce estimates of the system performance scores. In the end, systems are ranked as usual, but with some degree of error which is inversely proportional to the number of available annotations.

The plan is as follows. At the beginning, our confidence on the results is minimal, because we start with a small number of annotations, if any at all, and thus the variance is maximal. Participants may submit their system's output, for which we can offer an estimate of performance. At any point during the evaluation round, or even months or years later, we may receive new annotations for the missing parts of the dataset, which will result in better estimates of system performance, reduce variance and provide more confidence on the results. Therefore, the dataset is built incrementally: we can estimate system performance with whatever annotations we have, and as we keep receiving new annotations, our estimates get better and better. A wealth of research has shown that, when applying appropriate methods, evaluating with an incomplete ground truth is perfectly feasible, even when only a very small fraction of the annotations are known, say, 5\%. Well known examples are the text~\cite{Carterette2007:robust,Yilmaz2008:simple} and video IR communities~\cite{trecvid2014:overview}.
