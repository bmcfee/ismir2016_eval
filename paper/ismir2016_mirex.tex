...
Similarly, a proposed system, $\mathcal{F}$, operates on the same input, producing a prediction, $\hat{Y}_i$.
...
Usually, there is no access to any $Y_i$, and in most cases there is no access to $X_i$ either, making it impossible to diagnose why an algorithm produced $\hat{Y}_i$, or determine what should be done to make $\mathcal{F}$ better.
