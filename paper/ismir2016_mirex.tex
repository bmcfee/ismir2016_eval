...
MedleyDB, a well-curated dataset of over 100 multitracks for a number of MIR tasks, cost approximately \$12 a
track (\$1240 total)~\cite{bittner2014medleydb};
...
For reference, the ISMIR Society maintains a membership of roughly 250 individuals each year.
...
Popular websites such as Genius\footnote{\url{http://genius.com/}} (6M monthly active users) and
Ultimate~Guitar\footnote{\url{http://www.ultimate-guitar.com/}} (3M monthly active users) demonstrate the existence of these communities for lyrics and guitar tablature, respectively.\footnote{Data gathered from \url{http://compete.com}, March 2016}
...
We conclude our proposal by advocating a large-scale instrument identification task for ISMIR2017.
% What is the task?
In this task, one or more active instrument classes are predicted from a pre-defined taxonomy over an input recording.
Some of the effort in building an instrument taxonomy can be leveraged from WordNet~\cite{miller1995wordnet}, and refined by the community, starting at this year's ISMIR conference.
% Motivations
...
Instrument identification is an important, classic problem in MIR, but is not currently included in MIREX.\@
...
Compared with other music perception tasks, annotation requires a simple interface, \eg, ``check all that apply''.
The task enjoys a modicum of objectivity: a particular instrument is either present in the mix or not.%
